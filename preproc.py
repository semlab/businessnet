...
class TripletsFilter():
    """Filter triplets within files generated by OpenIE and filter to keep the 
       ones containing entities of interest in their subject and object. 
    """

    def __init__(self, infolder, outfolder):
        self.infolder = infolder
        self.outfolder = outfolder
        self.nlp = LangModel.get_instance()


    def filter(self):
        for filename in os.listdir(self.infolder):
            if filename.endswith(".csv"):
                infile = open(os.path.join(self.infolder, filename), 'r')
                outfile = open(os.path.join(self.outfolder, filename), 'w')
                csv_reader = csv.reader(infile)
                csv_writer = csv.writer(outfile)
                for row in csv_reader:
                    subj_doc = self.nlp(row[1])
                    obj_doc = self.nlp(row[3])
                    subj_ents_labels = {e.label_ for e in subj_doc.ents}
                    obj_ents_labels = {e.label_ for e in obj_doc.ents}
                    sub_set = subj_ents_labels.intersection(NodeType.Set)
                    obj_set = obj_ents_labels.intersection(NodeType.Set)
                    if len(sub_set) > 0 and len(obj_set) > 0:
...
if __name__ == "__main__":
    # TODO Select action with cmd argument
    ...
    infolder = "./data/csv/"
    outfolder = "./data/csv_flt/"
    flt = TripletsFilter(infolder, outfolder)
    flt.filter()
